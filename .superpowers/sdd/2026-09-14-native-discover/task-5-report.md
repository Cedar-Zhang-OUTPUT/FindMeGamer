...
## Independent-review correction: stale batch refresh

Review found an important race: a batch-list poll started before a new accepted submission could replace the cache with its older completed-only snapshot and remove the record from polling. Fixed only this concern by tracking accepted-submission revisions per Discover record. A refresh captures that revision before awaiting the API and discards its result if an intervening submission was acknowledged, leaving both acknowledged cache and tracking intact for the next poll.

TDD RED command: `swift test --package-path macos --no-parallel --filter staleBatchPollCannotEraseAcknowledgedBatchOrStopMatchPolling` — **1 test failed with 2 issues** (9.105 seconds): the newest running batch was absent after releasing the gated old response, and its eventual Match ID was nil. Preserved log: `/tmp/fmg-task5-batch-race-red.log`.

Covering GREEN command: `swift test --package-path macos --no-parallel --filter 'DiscoverModelTests|DiscoverServiceTests'` — **9 tests in 2 suites passed** (6.045 seconds), exit 0. Preserved log: `/tmp/fmg-task5-batch-race-green.log`. The regression runs the real model polling loop: suspend an old poll, acknowledge a second batch, release the completed-only old snapshot, verify the new batch remains active, then verify the next poll exposes its Match ID without reopening. `git diff --check` passed. No full-suite rerun, GUI exploration, backend edits, or release actions were performed for this narrow correction. The minor recovered-polling-error display concern remains deferred as requested.
