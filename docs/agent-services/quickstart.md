...
YouTube/X/Twitch/Steam API reads, public business email enrichment and templated sends are included. Twitch supports public research reads and follower totals, not privileged follower lists. No legacy Library/Match database, social platform writes, Instagram API, reply ingestion, Yes/No callbacks or attachment sending is claimed. Unspecified discovery targets default to 10 suitable creators per supported selected platform, or 30 across YouTube/X/Twitch when platforms are unspecified.
