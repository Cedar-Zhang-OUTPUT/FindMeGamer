...
YouTube/X/Twitch/Steam API reads, public business email enrichment and templated sends are included. Twitch supports public research reads and follower totals, not privileged follower lists. New emails are plain text only. Outreach tasks support real mailbox replies when the administrator configures IMAP; check monitor health before interpreting reply counts. Yes/No callbacks are retired. No legacy Library/Match database, social platform writes, Instagram API or attachment sending is claimed. Unspecified discovery targets default to 10 suitable creators per supported selected platform, or 30 across YouTube/X/Twitch when platforms are unspecified. See [mailbox setup and rollout](inbox-monitor.md).
