...
2026-09-14 补充：经确认的 IG 人工导入可以使用 `ig-<小写用户名>` 作为临时身份键，`account_id_source_url` 必须留空。它不是官方平台 ID，系统会明确标记为 provisional，并保留自己的 Profile UUID。重复用户名不会因为后来填了数字 ID 就自动新增或覆盖原 Profile；真实 ID 关联另行处理。Twitch 仍沿用数字 ID 规则，空 ID 仍不能直接写入。
