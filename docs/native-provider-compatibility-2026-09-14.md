...
- `e43b6ed8e35b4480803bc285eb83d1bf9010db14`: selectively adapted existing Creator schema fields to 4,000 characters for prose/reasons/observations and 512 characters for descriptive list items. Creator map/reduce stages retain 32,000-byte aggregate guards; the published Creator Brief retains an 8,000-byte aggregate guard. Type, nonblank, uniqueness, evidence count and exact citation validation remain. Game Brief remains bounded at 16,000 bytes. Screening and its empty-result recheck now have a 916,000-byte total input budget: 100 × 8,000-byte Creator Briefs + 16,000-byte Game Brief + 100,000 shared bytes for IDs, framing and manual context. Manual edits are subject to that shared whole-request allowance; arbitrary-size overrides do not gain unlimited screening capacity. Ranking retains its 512,000-byte total budget; pairwise and individual message limits are unchanged. Focused gateway tests preserve the complete screening input through a recheck and schema repair, with explicit oversized-request rejection.
