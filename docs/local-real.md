...
YouTube, DeepSeek, Google AI Studio, and SMTP remain real integrations and are
configured from the app's Settings screen when needed.
...
For the current v1 smoke flow, configure YouTube and DeepSeek in Settings. Also
configure Google AI Studio to enable Gemini public-web email research whenever
the original channel description and linked pages contain no usable email.
Steam Store analysis uses the public Store endpoint, so leave the optional
Steam credential empty; its Settings connection test is not used by this flow.
