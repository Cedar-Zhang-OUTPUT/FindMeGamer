...
- **追加验收**：名单/准备批次 `8cabb11` 已独立复验并接受（B7）；其余页面基线不变。共享采集开关与新版 Match 界面仍在制。测试夹具 `4bb9d78` 是验收设施，不是这些业务功能交付。
- **最新增量**：共享采集开关 `b2b15f4` 已接受（B8）。Match客户端 `149bc69` 已固定快照通过源码验收（F4），但最终打包E2E仍待钥匙串授权，不是整单元已完成；后续Settings开关UI及P4/P12完整查询已继续独立推进。
...
1. **先完成已在制的两个单元。** 后端名单准备已接受，现独立推进共享渠道开关；前端 Match 选择游戏/参考作品、P3 条件、规划/发现/评价及统一详情。总管固定 `cad5565` API+Worker fixture 已独立通过10项夹具测试及成功/故障smoke，并正式交给前端；不替代桌面业务验收。
