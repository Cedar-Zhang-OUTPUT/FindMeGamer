...
| [P4 用户选择、冻结 N 位](https://k1mai98sti.feishu.cn/docx/MOoWdOYNvoogWnxF7jYcGgGinrh#doxcnK489mc06ci75VcIPfIikHg) → [P6 缺项保留](https://k1mai98sti.feishu.cn/docx/MOoWdOYNvoogWnxF7jYcGgGinrh#doxcncrNXorGNjuNQpIxsrYNmNg) | 必须跨所有已加载分页全选当前筛选结果；追加默认不选并提示新增数；筛选变化保留仍可见选中项并提示移出数，排序不改变选择。批量发信先停止后续查找并冻结当前账号集合，迟到账号不纳入。**缺邮箱/证据仍属于 N 位，进入准备后标需修复；准备快照允许 chosen email 为空，不能静默丢成员或先有邮箱才允许进入。**多个邮箱只明确选择一个。准备冻结不是最终发送授权。 | B4/B6 基线 `selected=false`；当前后端在制选择/准备单元未计入 accepted | `in-progress`：后端当前 Activity 名单/邮箱/准备批次单元；前端待已接受契约接入，不凭空发明接口。 |
| [P4 名单保存、查询隔离、排序与证据过滤](https://k1mai98sti.feishu.cn/docx/MOoWdOYNvoogWnxF7jYcGgGinrh#doxcnkbXL4bsv7Uj1blrISCRyAd) | 新查询已有独立结果集合/旧游标保留基础。仍需“有名称的候选集合”保存/恢复；五类作品证据筛选（全部/当前游戏/参考作品/其他相关/暂无）；相关性、粉丝、最近发布、最近加入四种排序，缺失在后。基线发现按加入顺序、评价按内部得分排序，不能据此宣称全部原文选项已支持。 | B4、B6；[Activity API](../backend/app/api/routes/activity.py)、[评价结果 API](../backend/app/api/routes/discovery_evaluation.py) | `planned`：后端名单查询能力 + 前端完整控制；与当前选择单元协调后有限补齐。 |
...
| [P12.1 完整筛选、排序与行摘要](https://k1mai98sti.feishu.cn/docx/MOoWdOYNvoogWnxF7jYcGgGinrh#doxcnxyKvfIDWqRswf5mjEPUZ0f) | 原文要求与 P3 同套平台/语言多选和枚举、四种排序、行内近期作品/联系情况/最近更新。基线 [CreatorLibrary](../desktop/src/renderer/components/creators/CreatorLibrary.tsx) 是单平台 select、单语言输入，列表未展示这些完整摘要，不能以 API 当前字段少为由删除需求。应按全体分页结果排序/筛选，不能仅改当前页顺序。 | B2 list 契约；固定基线 CreatorLibrary | `planned`：后端 Library 查询扩展/必要摘要 + 前端控件；作为完整 PRD 补齐单元，保留 F3 已接受写入/详情结论。 |
...
| [P12.2 官网状态、排序与 Match/Steam 入口](https://k1mai98sti.feishu.cn/docx/MOoWdOYNvoogWnxF7jYcGgGinrh#doxcnDooFzqEExqIRD60OtD45Ph) | 仍需全部/有官网/无官网筛选；最近更新/最近加入/名称 A–Z 排序；“用此游戏匹配”先入 P2.1；添加游戏同页分 Steam/手填。基线 [Game API](../backend/app/api/routes/library_v2.py) 仅 query/only_collection/分页，固定名称/官网排序；[GameLibrary](../desktop/src/renderer/components/GameLibrary.tsx) 不含完整原文选择。 | B1、F1 明确的现有范围；无新版全选项验证 | Match 入口 `in-progress`（前端）；筛选/排序及 Steam 分支 `planned`（后端/前端相应补齐单元）。 |
...
| Settings 原先空容器；现有自动更新只有独立周期 | 新增**云端共享的逐渠道采集 on/off**（YouTube/X/Twitch/Instagram），同时保留每次 Match 的平台选择。关闭后停止该渠道后续采集，含该渠道 reanalysis；保留既有资料及匹配/外联，其他渠道继续 | `approved change`，**尚未实现**。后端新增共享配置与采集/再分析统一执行检查；前端独立 Settings 单元随后接入。开关开启不使 Twitch/Instagram 自动变成可用提供商，也不是全局关闭匹配或删除数据。 |
...
| 安装包 / 发布 | 当前最终本机 arm64 `.app` 已有严格 deep ad-hoc 签名、图标及打包 E2E 记录；9 个 runtime 文件与最终 build 字节一致由总管核对 | `accepted` 仅限本机包。Intel、macOS 14 实机、DMG、下载后 Gatekeeper、公证及正式发行均未验收。内部安装交付列最终有限单元；当前无 GitHub 上传、Release、部署或真实外发动作。 |

## 剩余工作顺序与结项条件

1. **先完成已在制的两个单元。** 后端 Activity 选择/单邮箱/保留缺项的准备批次；前端 Match 选择游戏/参考作品、P3 条件、规划/发现/评价及统一详情。总管固定 `cad5565` API+Worker 和 fixture 环境；环境准备不等于业务验收。
2. **补齐名单与必要入口。** P4 命名名单、证据筛选/排序、选择跨追加/分页/筛选/返回语义；P12 完整查询/摘要；Steam 自动解析/手填、必要 Analyze/证据获取入口。按可独立验证的契约/UI 单元交付，不因当前 API 缺项删除原要求。渠道采集开关作为独立后端/Settings 增量，可在安全点安排。
