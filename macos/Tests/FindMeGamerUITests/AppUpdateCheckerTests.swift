...
@Suite struct AppUpdateCheckerTests {
  ...
  func liveApprovedManifestOffers014To012And013AndKeeps014Current() async throws {
    for installed in ["0.1.2", "0.1.3", "0.1.4"] {
      let preferences = UpdateTestPreferences()
      defer { preferences.cleanUp() }
      let checker = AppUpdateChecker(
      ...
      if installed != "0.1.4" {
        ...
        #expect(available.version == "0.1.4")
        #expect(available.releasePageURL.absoluteString == releasePage(version: "0.1.4"))
      } else {
        #expect(checker.availableRelease == nil)
      }
    }
