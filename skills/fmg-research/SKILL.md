...
Use a local workspace keyed by Steam App ID and a stable run ID for all metered CLI calls. Record progress after useful steps. Follow the user's task rather than mechanically rerunning a pipeline: direct-account evaluation, contact-only work, explanation and resume can use saved evidence without new searches.
...
End each run with results, evidence limitations, file locations, used request budget and `fmg --run-id ID usage`. Preserve partial work and failures. Money and Codex/Work per-task usage remain unknown when not attributable; never substitute shared-account changes. Actual sending requires approval of the recipient/content batch; drafting alone is not approval.
