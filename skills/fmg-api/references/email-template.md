...
I’m reaching out because I’d love to invite you to try our game, ${game_name}. ${availability_statement}
...
- `game_name`: Confirmed target game title, not the creator's reference game.
- `game_tagline`: Short factual phrase for the subject; no final punctuation or line breaks.
- `game_summary`: Complete concise paragraph with punctuation: confirmed genre, style, premise. Do not copy facts from another game.
- `gameplay_summary`: Complete concise paragraph with punctuation: actual gameplay and game loop; avoid repeating game_summary.
- `availability_statement`: Complete factual sentence with punctuation explaining current availability or where to learn more; never invent demo, release or access.
- `game_url`: Verified public link for this target game or its available demo.
