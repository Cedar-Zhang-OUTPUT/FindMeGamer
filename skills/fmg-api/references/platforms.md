...
Use category discovery → streams/videos/clips → unique broadcaster IDs → batched profile/channel reads, with `searchChannels` as an additional channel-name lead. Twitch IDs differ from Steam IDs. On clips, `broadcaster_id` is the stream owner; `creator_id` is the person who clipped it. `getChannelFollowers.total` is the follower count; `data: []` does not mean zero followers. Detailed follower lists require additional broadcaster/moderator permissions not included here. `getUsers.view_count` is deprecated; don't use it. `getUsers.email` cannot reveal arbitrary creators' private email. A canonical profile is `https://www.twitch.tv/<login>`.
