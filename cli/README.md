...
The ledger records platform attempts, SMTP attempts, and each Gemini enrichment attempt with available numeric token usage. In-progress/interrupted attempts remain visible. It excludes message text, provider credentials and API response bodies. Recognized list counts are returned; unfamiliar response shapes do not become zero items. Versioned estimated USD snapshots appear in `meta.cost` on platform responses and `checkpoints.enrichment.cost` on completed enrichment. `usage` returns per-provider and total known estimated subtotals, detailed excluded components and `complete_cost_known`. Actual billing remains null: list-price estimates are before X daily deduplication, discounts/free allowances, and exclude hosting/subscriptions. YouTube quota is separate. Codex/Work precise task usage remains null unless the host supplies genuine task measurements; the Skills separately report available beginning/ending account quota snapshots.

## Automated updates

`fmg upgrade --latest --skills` updates this CLI and matching Skills with the verified installer (Python 3.10+), keeping authentication and numbered backups of replaced Skills. Use `--skill-dir DIRECTORY` for an existing custom Skill directory, or `--tag fmg-v0.2.0` to pin a version. Without `--skills`, only the binary changes. Reload Skill instructions after updating. CLI 0.1.0 can bootstrap using the new release installer with `--skills`; follow the public Agent guide rather than choosing a legacy macOS DMG as GitHub latest.
