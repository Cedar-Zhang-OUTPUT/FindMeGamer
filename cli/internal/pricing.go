...
func pricingRules() map[string]any {
	return map[string]any{
		...
		"providers": map[string]any{
			"twitch":     map[string]any{"estimated_marginal_api_usd": "0", "rule": "Standard Helix reads: current zero marginal API charge assumption, not unlimited usage or an invoice. Respect returned Ratelimit headers and endpoint limits; app and user token buckets differ. OAuth is server-managed. No public creator-country filter or third-party private email access.", "source": "https://dev.twitch.tv/docs/api/guide/"},
