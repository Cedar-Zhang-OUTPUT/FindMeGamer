...
- Preload: typed `connection`, `library`, `creators`, `games`, `match`, `savedSets`, `outreach`, `drafts`, `settings`, `preferences`, `updates`, and `openExternal` business methods. No generic fetch, IPC channel, Node, filesystem or shell access.
...
- Draft/template email preview: sandboxed iframe with no scripts, forms, external requests or business IPC. Locked fixed content is preserved; drafting is implemented, sending is not part of P6.
