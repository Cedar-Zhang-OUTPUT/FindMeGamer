...
- Activity outreach preparation: explicit human selection, one chosen email or None, name confirmation and recorded-work selection. Prepare N stops discovery before freezing the ordered people, keeps incomplete members, and opens a current-repair workspace with immutable original snapshots and read-only history.
...
The standalone Outreach entry remains **Not connected yet**; preparation, templates, drafts and final sending live inside their Activity in Match. Steam/Analyze Request UI is not exposed yet. Match requires the separately accepted v2 discovery/evaluation backend; the older Library/Settings fixture does not implement it. Match exposes YouTube and X for one run, with Twitch/Instagram visibly unavailable. Settings can send one SMTP test email only after explicit recipient confirmation. Steam credentials can be saved but testing is unavailable in the accepted backend; X credential testing checks usage access only, not recent search, balance or analysis.
...
| Explicit outreach people | `GET/POST /api/v2/activities/{uuid}/selections`, `GET .../selections/{uuid}`, `POST .../selections/bulk`, `POST .../selections/{uuid}/update`, `POST .../selections/{uuid}/cancel`; narrow revision/context-checked writes with idempotency keys |
| Frozen preparation | `GET/POST /api/v2/activities/{uuid}/recipient-batches`, `GET .../recipient-batches/{uuid}`; ordered 1–600 recipient IDs/revisions/context tokens and persistent request ID |
...
- Preload: typed `connection`, `library`, `creators`, `games`, `match`, `savedSets`, `outreach`, `drafts`, `sending`, `settings`, `preferences`, `updates`, and `openExternal` business methods. No generic fetch, IPC channel, Node, filesystem or shell access.
...
- Draft/template/delivery email preview: sandboxed iframe with no scripts, forms, external requests or business IPC. Locked fixed content is preserved. P7 adds explicit sending separately from P6 drafting.
...
Baseline source checks:803 tests/57 files, typecheck/build and opt-in HTTP1/1 passed. The separate hidden-Settings supplement passed805 tests/58 files, typecheck/build on an isolated baseline-plus-five-file snapshot. Settings reads now occur only in the active cloud category, with drafts retained. After authorized installation of matching managed Chromium, `FMG_QUERY_RENDERER_FIXTURE_FILE=/absolute/path/to/authorized/client.json npx playwright test e2e/query-renderer.spec.ts` passed1/1 against real HTTP: keyboard, filters/order, save/restore, focus/scroll return, wide/narrow/135% font/reduced-motion geometry and clean console. Original failures are preserved in the [unit record](docs/query-lists-unit.md). Native packaged-app/IPC/Keychain acceptance remains pending; HEADLESS is not a native pass.

F7's full source regression passed924 tests/69 files before the final runtime-discovered history timing fix. That minimal fix then passed35 related tests/5 files, typecheck and production build; a new full-suite result is not implied. The real HTTP preparation flow passed all nine methods, including stale conflicts, cross-query identity deduplication, incomplete N, idempotent freeze and immutable history. A separately authorized two-contact supplement verified explicit second-email selection and source-version re-confirmation. See [outreach-preparation-unit.md](docs/outreach-preparation-unit.md) for exact side effects, review fixes, runtime evidence and limits. Explicit human selections remain distinct from **Save list** marks: sorting/restoring does not mutate them; an explicit filter change removes only excluded members of the previously visible selection projection after a complete server-membership read.

F7's final history-only production-renderer check passed1/1 with25 GET and zero writes, one batch-detail read, keyboard snapshot access, wide/narrow/135%-font/reduced-motion checks and clean console. Earlier full-flow attempts and their limits remain recorded separately; neither this browser pass nor prior HTTP acceptance claims native packaged-app/IPC/Keychain acceptance.

F8 adds **Choose template → Create N drafts** inside the current Activity. Catalog/history reads do not register templates or generate mail. Existing versions are immutable; the original LIMINAL template is explicitly registered only for its accepted game identity, while new versions are separately saved. Every original recipient remains in order, including missing contact/evidence. The selected email, four color-linked values and nearby source repair are the focus; metadata, original snapshots and human sender confirmations are on demand. Ordinary person/people/Creator detours retain local edits. Source refresh explicitly clears values and confirmations; uncertain revision writes require readback, never automatic rebasing or blind replay. Persistent creations retain the same request UUID/key for explicit recovery. All P6 `send_ready` values remain false, with no Send button or SMTP action.

See [outreach-drafts-unit.md](docs/outreach-drafts-unit.md) for the state model, exact verification segments and limits, and [outreach-drafts-real-api-verification.md](docs/outreach-drafts-real-api-verification.md) for actual API/worker/model-fixture evidence. The real HTTP test is coordinator-lease-only and mutates its own labeled synthetic records. The separate headless renderer check is GET-only; neither substitutes for native packaging, IPC or Keychain acceptance.

F8's final frozen-source headless check passed36 GET/zero writes and clean runtime/bridge/network checks. Root inspected wide and760px/135%-font screenshots. Opening history hands focus to the draft set and collapses its secondary list; local mail edits survive people and Creator detours. Full source regression evidence is segmented:1,018/1,019 initially passed, one existing SavedSet test timed out and then its isolated file passed4/4; later targeted regression groups passed45 and20 cases with typecheck/build. The record does not imply a fresh single-run full-suite clean result after final changes.

## Final sending (B / P7)

**Review sending** checks every original recipient without sending mail. Actual From/Reply-To, recipient addresses and the selected complete email stay visible. Repair the relevant draft or explicitly exclude a person with a reason; recheck before **Send N emails**. SMTP settings are a retained same-workspace detour. Advanced sources and original exclusions remain available in nearby disclosures.

The six narrow sending APIs consume accepted B `ece2e9d` / migration0017, without backend changes. Final send freezes the exact qualification, durable request UUID and key. An uncertain submission can replay only that original request; no matching-looking history item proves request identity. Delivery retry and verification are bound to the observed attempt, not idempotency headers. Unknown SMTP outcomes have no normal Retry: explicit outcome plus source note records the finding, and recording not-sent never automatically resends. Sent means SMTP acceptance, not confirmed inbox delivery; manually verified sending is labeled separately.

Frozen requests and local exclusions/verification notes survive normal in-window detours, not process crashes. Recovery after a crash must inspect server history; the app does not invent a safe replacement submission. Same-attempt uncertain delivery changes remain locked unless an exact resolution receipt proves them; reviewing a strictly newer attempt is separate from proving the prior change succeeded. See [activity-sending-unit.md](docs/activity-sending-unit.md) for scoped verification and limitations. No native packaging or release is implied by source/headless checks.
