...
it('keeps one activity session when going from Match to Outreach and continuing discovery',async()=>{
 ...
 await user.click(screen.getByRole('button',{name:'Outreach'}));await screen.findByRole('button',{name:'Record response'});await user.selectOptions(screen.getByLabelText('Response',{selector:'select'}),'accepted');
