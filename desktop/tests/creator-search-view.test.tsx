...
it('retains previously loaded results after a read failure but disables selection',()=>{
 ...
 expect(screen.getByText('Match details unavailable')).toBeVisible();expect(screen.getByText('Could not refresh.')).toBeVisible();
});
...
it('shows local failures with read-only reload while protecting selection and business retry',async()=>{
 ...
 expect(onRefresh).toHaveBeenCalledTimes(1);expect(onRetry).not.toHaveBeenCalled();expect(onToggle).not.toHaveBeenCalled();
});
