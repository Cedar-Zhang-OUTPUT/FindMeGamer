...
test('Creator edits, contacts, known works and identity history survive real API writes and interrupted responses',async()=>{
  ...
  try{
    ...
    await page.getByRole('tab',{name:'Emails',exact:true}).click();await page.getByRole('button',{name:'Add email',exact:true}).click();await page.getByRole('textbox',{name:'Email',exact:true}).fill(email);await page.getByLabel('Purpose',{exact:true}).fill('Business');await page.getByRole('button',{name:'Add email',exact:true}).click();await page.getByRole('button',{name:'Retry same request',exact:true}).click();
    ...
    await page.getByRole('tab',{name:'Emails',exact:true}).click();await expect(page.getByRole('button',{name:`Edit ${email}`,exact:true})).toHaveCount(0);await expect(page.getByRole('heading',{name:email,exact:true})).toBeVisible();
    await page.getByRole('tab',{name:'Known works',exact:true}).click();await expect(page.getByRole('heading',{name:'No known works',exact:true})).toBeVisible();await page.getByLabel('Include previous identities',{exact:true}).check();await expect(page.getByRole('heading',{name:'Edited gameplay',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Edit Edited gameplay',exact:true})).toHaveCount(0);
