...
test('C built renderer saves only explicit notes and retains unsaved collaboration context', async ({ browser }, testInfo) => {
  ...
  try {
    ...
    if(globalReadOnly){
      ...
      await expect(page.getByRole('region',{name:'Find & prepare',exact:true})).toBeVisible();
      ...
      await page.getByRole('textbox',{name:'Content keywords',exact:true}).fill('local-only navigation check');await page.getByRole('button',{name:'Add keyword',exact:true}).click();
      await page.getByRole('button',{name:'Open in Outreach',exact:true}).click();await page.getByRole('dialog',{name:'Unsaved Match changes',exact:true}).getByRole('button',{name:'Keep working',exact:true}).click();
      await expect(page.getByRole('button',{name:'Remove keyword local-only navigation check',exact:true})).toBeVisible();
      await page.getByRole('form',{name:'Discovery conditions'}).getByRole('button',{name:'Cancel',exact:true}).click();
      await page.getByRole('button',{name:'Open in Outreach',exact:true}).click();
      await expect(responseFilter).toHaveValue('accepted');
      ...
      await editor.getByRole('button',{name:'Edit progress',exact:true}).click();await editor.getByRole('textbox',{name:'Notes',exact:true}).fill('Local-only unsaved Outreach check');
      await page.getByRole('button',{name:'Continue in Match',exact:true}).click();await page.getByRole('dialog',{name:'Unsaved Match changes',exact:true}).getByRole('button',{name:'Keep working',exact:true}).click();
      await expect(editor.getByRole('textbox',{name:'Notes',exact:true})).toHaveValue('Local-only unsaved Outreach check');await editor.getByRole('button',{name:'Cancel',exact:true}).click();
    ...
    await page.getByRole('tab', { name: 'Prepare & send', exact: true }).click(); await page.getByRole('tab', { name: 'Invitations', exact: true }).click(); await expect(notes).toHaveValue(unsaved); await expect(responseFilter).toHaveValue('accepted');
