...
test('internal package shows first-run cloud origin without a key or connection',async({},info)=>{
 ...
 if(!executable?.includes('/artifacts/FindMeGamer-Electron-0.2.0-internal.8-arm64-')||!executable.endsWith('/FindMeGamer.app/Contents/MacOS/FindMeGamer'))throw Error('explicit_internal_package_required');
 ...
 try{
  ...
  });expect(main).toMatchObject({packaged:true,userData,version:'0.2.0-internal.8'});
