...
test('game entry built renderer uses Library handoff and one source-only Steam import',async({browser},info)=>{
 ...
 try{
  ...
  await checkpoint('library_handoff');await page.goto(origin);await expect(page.getByText('Workspace linked',{exact:true})).toBeVisible();await page.getByRole('tab',{name:'Games',exact:true}).click();
  ...
  const p2=info.outputPath('P2-review-wide.png');await page.screenshot({path:p2});(report.screenshots as string[]).push(p2);
  await page.getByRole('button',{name:'Back to games',exact:true}).click();
  await expect(page.getByRole('heading',{name:'Choose a game',exact:true})).toBeVisible();await expect(page.getByRole('button',{name:'Use game '+before.name,exact:true})).toBeVisible();
  await expect(page.getByLabel('Activity name',{exact:true})).toHaveCount(0);await expect(page.getByText('Linked game',{exact:true})).toHaveCount(0);
  const p1=info.outputPath('P1-library-wide.png');await page.screenshot({path:p1});(report.screenshots as string[]).push(p1);
  await page.getByRole('searchbox',{name:'Search games',exact:true}).fill('Retained search');await page.getByRole('tab',{name:'Steam',exact:true}).click();
  ...
  const steam=info.outputPath('P1-steam-wide.png');await page.screenshot({path:steam});(report.screenshots as string[]).push(steam);
  if(process.env.FMG_GAME_ENTRY_READ_ONLY==='64692'){
   await page.getByRole('tab',{name:'Library',exact:true}).click();await page.setViewportSize({width:760,height:920});await page.evaluate(()=>document.documentElement.style.fontSize='135%');
   requireCheck(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),'narrow_overflow');const narrow=info.outputPath('P1-library-narrow.png');await page.screenshot({path:narrow});(report.screenshots as string[]).push(narrow);
   requireCheck(traffic.every(r=>r.method==='GET')&&!pageErrors&&!unexpectedBridge&&!forbidden,'readonly_runtime_scope');Object.assign(report,{status:'passed',stage:'complete',checks:['library_handoff_review_no_write','source_tabs_retain_inputs','narrow_135_reduced_motion'],source_import:'not_run_readonly'});
  }else{
  ...
  await expect(page.getByRole('button',{name:'Continue to matching',exact:true})).toBeVisible();await page.setViewportSize({width:760,height:920});await page.evaluate(()=>{document.documentElement.style.fontSize='135%';});
