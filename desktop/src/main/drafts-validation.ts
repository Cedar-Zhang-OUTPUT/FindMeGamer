...
function stamp(v: unknown): string { const s = text(v, 'response', 100, 1); if (!/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.\d+)?(?:Z|[+-]\d\d:\d\d)$/.test(s) || !Number.isFinite(Date.parse(s))) fail('response'); return s; }
...
function content(v: unknown, extra: string[]): Record<string, unknown> {
  ...
  const source = exact(r.source_metadata, ['kind', 'document_id', 'revision', 'steam_app_id', 'raw_hash'], m);
...
export function decodeTemplate(v: unknown, id?: string, gameId?: string): DTO.TemplateVersion { const r = content(v, ['id', 'game_id', 'created_at']); const actual = identifier(r.id, 'response'), game = identifier(r.game_id, 'response'); if ((id && !same(id, actual)) || (gameId && !same(gameId, game))) fail('response'); stamp(r.created_at); return r as unknown as DTO.TemplateVersion; }
export function decodeCatalog(v: unknown, gameId: string): DTO.TemplateCatalog { const r = exact(v, ['items', 'builtin'], 'response'); const items = list(r.items, 'response', 10_000, v => decodeTemplate(v, undefined, gameId)); unique(items.map(v => v.id), 'response'); const builtin = content(r.builtin, ['key', 'requires_explicit_registration']); if (builtin.key !== 'liminal-revision-69' || builtin.requires_explicit_registration !== true || (builtin.source_metadata as DTO.TemplateSource).kind !== 'canonical') fail('response'); return { items, builtin: builtin as unknown as DTO.BuiltinTemplate }; }
function jsonObject(v: unknown): JsonObject { let nodes = 0; const seen = new WeakSet<object>(); function check(v: unknown, depth: number): void { if (++nodes > 20_000 || depth > 20) fail('response'); if (v === null || typeof v === 'boolean') return; if (typeof v === 'string') { text(v, 'response', 1_000_000); return; } if (typeof v === 'number') { if (!Number.isFinite(v)) fail('response'); return; } if (typeof v !== 'object' || seen.has(v)) fail('response'); seen.add(v); if (Array.isArray(v)) { if (v.length > 10_000) fail('response'); v.forEach(i => check(i, depth + 1)); } else for (const [k, item] of Object.entries(object(v, 'response'))) { if (['__proto__', 'constructor', 'prototype'].includes(k)) fail('response'); text(k, 'response', 512); check(item, depth + 1); } seen.delete(v); } object(v, 'response'); check(v, 0); return v as JsonObject; }
