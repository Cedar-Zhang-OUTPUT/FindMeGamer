...
export function SourceImport({api,target,onSaved,onClose,onNavigationGuardChange,onConnectionRepair,onLockChange,allowArchive=true}:{api:DesktopBridge;target:SourceTarget;onSaved:(record:GameDetail|CreatorDetail)=>void;onClose:()=>void;onNavigationGuardChange?:(guard:NavigationGuard|null)=>void;onConnectionRepair?:()=>void;onLockChange?:(locked:boolean)=>void;allowArchive?:boolean}) {
 ...
 {target.kind==='game'&&state.kind==='failed'&&<button type="button" className="button secondary" onClick={()=>setManual(true)}>{target.game?'Edit saved game manually':'Enter manually'}</button>}
