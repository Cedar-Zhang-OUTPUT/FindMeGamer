...
export function SourceImport({api,target,onSaved,onClose,onNavigationGuardChange,onConnectionRepair}:{api:DesktopBridge;target:SourceTarget;onSaved:(record:GameDetail|CreatorDetail)=>void;onClose:()=>void;onNavigationGuardChange?:(guard:NavigationGuard|null)=>void;onConnectionRepair?:()=>void}) {
 ...
 </form>{leaving&&<div role="alertdialog" aria-label="Leave source import"><h3>{locked?'Resolve this source request first':'Leave source import?'}</h3><button className="button secondary" onClick={()=>{setLeaving(false);pendingNavigation.current=null;}}>Keep importing</button>{!locked&&<button className="button secondary" onClick={()=>{const proceed=pendingNavigation.current;pendingNavigation.current=null;setLeaving(false);onClose();proceed?.();}}>Discard link and leave</button>}</div>}</section>;
}
