...
export function GameLibrary({ api, active, onNavigationGuardChange, onConnectionRepair }: { api: DesktopBridge; active: boolean; onNavigationGuardChange?: (guard: NavigationGuard | null) => void; onConnectionRepair?: () => void }) {
  ...
  return <div className="games-library">
    <div ref={listRoot} hidden={route.kind !== 'list'}><div className="game-library-tools"><form className="search-form" role="search" onSubmit={event => { event.preventDefault(); void loadPage({ query: library.search.trim(), onlyCollection: library.onlyCollection, offset: 0 }, false); }}><Icon name="search"/><input type="search" aria-label="Search games" maxLength={255} placeholder="Search games…" value={library.search} onChange={event => setLibrary(previous => ({ ...previous, search: event.target.value }))}/><button className="button secondary" type="submit">Search</button></form><button className="button primary" onClick={() => { rememberScroll(); scrollTop(); setRoute({ kind: 'editor', game: null }); }}>New game</button></div>
      ...
      {page && page.items.length > 0 && <ul className="profile-list" aria-label="Games">{page.items.map(game => <li key={game.id}><button className="profile-row" aria-label={`Open ${titleOf(game)}`} onClick={() => void openGame(game.id)}><Artwork url={game.cover_url} name={titleOf(game)} kind="games"/><span className="profile-row-content"><span className="profile-row-title">{titleOf(game)}{game.favorite && <Icon name="collection"/>}</span><span className="profile-row-summary">{game.developer || game.website_url || 'Manual game'}</span>{game.tags.length > 0 && <span className="row-tags">{game.tags.slice(0, 3).map((tag, index) => <span key={index}>{tag}</span>)}</span>}</span>{game.last_analyzed_at && <span className="row-date">{analyzedDate(game.last_analyzed_at)}</span>}<Icon name="chevron"/></button></li>)}</ul>}
