...
function GameRecord({ api, game, saved, onBack, onEdit,onImport,onUseForMatch }: { api: DesktopBridge; game: GameDetail; saved?: boolean; onBack: () => void; onEdit: () => void;onImport:()=>void;onUseForMatch?:(game:GameDetail)=>void }) {
  ...
  return <article className="game-record"><div className="game-editor-heading"><button className="text-button" onClick={onBack}><Icon name="arrow"/>Back to games</button>{saved && <span className="game-saved-message" role="status"><Icon name="check"/>Saved</span>}</div>
    ...
    <div className="analysis-entry-actions">{game.source_identity.canonical_url?<button className="button primary" onClick={()=>analyze.request({target_type:'game',url:game.source_identity.canonical_url!,mode:'reanalyze',profileId:game.id,title:titleOf(game)})}>Analyze game</button>:null}<button className="button secondary" onClick={onImport}>{game.source_identity.steam_app_id?'Refresh Steam source':'Bind Steam source'}</button></div>
    {game.last_analyzed_at&&<GameAnalysisInsights api={api} id={game.id} revision={game.revision}/>}
    <div className={`game-detail-columns ${game.reference_works.length || ['partial','unavailable'].includes(game.steam_recommendations?.status ?? '') ? '' : 'game-detail-single'}`}><section>{game.description && <><h3>About</h3><p className="game-description-text">{game.description}</p></>}{game.tags.length > 0 && <ul className="tags">{game.tags.map((tag, index) => <li key={`${tag}-${index}`}>{tag}</li>)}</ul>}<dl className="game-identity-fields"><div><dt>Website</dt><dd>{game.website_url ? <><span>{game.website_url}</span>{canOpen(game.website_url) && <button className="text-button" onClick={() => void open(game.website_url!)}>Open website<Icon name="external"/></button>}</> : 'Not recorded'}</dd></div>{game.steam_app_id && <div><dt>Steam ID</dt><dd>{game.steam_app_id}</dd></div>}{game.languages.length > 0 && <div><dt>Languages</dt><dd>{game.languages.join(', ')}</dd></div>}</dl></section>
      {(game.reference_works.length > 0 || ['partial','unavailable'].includes(game.steam_recommendations?.status ?? '')) && <section className="game-detail-references"><h3>Reference works <span className="count">{game.reference_works.length}</span></h3><SteamRecommendationsStatus state={game.steam_recommendations}/>{game.reference_works.length ? <ul>{game.reference_works.map((reference, index) => <li key={reference.id || index}><h4>{reference.name || reference.url}</h4><SteamReferenceSource reference={reference} api={api}/>{reference.url && <div className="game-reference-link"><span>{reference.url}</span>{canOpen(reference.url) && <button className="text-button" onClick={() => void open(reference.url!)}>Open reference<Icon name="external"/></button>}</div>}{reference.similarities.length > 0 && <ul className="tags">{reference.similarities.map((item, itemIndex) => <li key={itemIndex}>{item}</li>)}</ul>}{reference.reason && <p>{reference.reason}</p>}</li>)}</ul> : <p className="muted">No references added.</p>}</section>}</div>
...
export function GameLibrary({ api, active, onNavigationGuardChange, onConnectionRepair,openRequest,onUseForMatch,onViewChange }: { api: DesktopBridge; active: boolean; onNavigationGuardChange?: (guard: NavigationGuard | null) => void; onConnectionRepair?: () => void;openRequest?:{id:string;nonce:number};onUseForMatch?:(game:GameDetail)=>void;onViewChange?:(view:'list'|'record')=>void }) {
  ...
  useEffect(()=>{if(openRequest)void openGame(openRequest.id);},[openRequest]);
  ...
  return <div className="games-library">
    {route.kind==='list'&&<div className="analysis-entry-actions"><button className="button secondary" onClick={()=>{rememberScroll();setRoute({kind:'source'});}}>Import from Steam</button></div>}
