...
import { GAME_FIELDS, type GameDetail, type GameListInput, type GamePage, type NavigationGuard } from '../../shared/games';
...
interface LibraryState { search: string; query: string; onlyCollection: boolean; page: GamePage | null; busy: boolean; error: PublicError | null; failedInput: GameListInput | null }
...
function GameRecord({ api, game, saved, onBack, onEdit }: { api: DesktopBridge; game: GameDetail; saved?: boolean; onBack: () => void; onEdit: () => void }) {
  ...
  return <article className="game-record"><div className="game-editor-heading"><button className="text-button" onClick={onBack}><Icon name="arrow"/>Back to games</button>{saved && <span className="game-saved-message" role="status"><Icon name="check"/>Saved</span>}</div>
    <section className="profile-identity"><Artwork url={game.cover_url} name={titleOf(game)} kind="games" large/><div className="identity-main"><span className="eyebrow">Game profile</span><h2 ref={heading} tabIndex={-1}>{titleOf(game)}</h2>{game.developer && <p>{game.developer}</p>}</div><button className="button primary" onClick={onEdit}>Edit game</button>
      <div className="profile-metrics"><div><span>Collection</span><strong>{game.favorite ? 'Saved' : 'Not saved'}</strong></div><div><span>Analyzed</span><strong>{analyzedDate(game.last_analyzed_at)}</strong></div>{game.release_date && <div><span>Release date</span><strong>{game.release_date}</strong></div>}</div>
    </section>
    {externalError && <ErrorNotice error={externalError}/>}
    <div className="game-detail-columns"><section><h3>About</h3><p className="game-description-text">{game.description || 'No description recorded.'}</p>{game.tags.length > 0 && <ul className="tags">{game.tags.map((tag, index) => <li key={`${tag}-${index}`}>{tag}</li>)}</ul>}<dl className="game-identity-fields"><div><dt>Website</dt><dd>{game.website_url ? <><span>{game.website_url}</span>{canOpen(game.website_url) && <button className="text-button" onClick={() => void open(game.website_url!)}>Open website<Icon name="external"/></button>}</> : 'Not recorded'}</dd></div>{game.steam_app_id && <div><dt>Steam ID</dt><dd>{game.steam_app_id}</dd></div>}{game.languages.length > 0 && <div><dt>Languages</dt><dd>{game.languages.join(', ')}</dd></div>}</dl></section>
      <section className="game-detail-references"><h3>Reference works <span className="count">{game.reference_works.length}</span></h3>{game.reference_works.length ? <ul>{game.reference_works.map((reference, index) => <li key={reference.id || index}><h4>{reference.name || reference.url}</h4>{reference.url && <div className="game-reference-link"><span>{reference.url}</span>{canOpen(reference.url) && <button className="text-button" onClick={() => void open(reference.url!)}>Open reference<Icon name="external"/></button>}</div>}{reference.similarities.length > 0 && <ul className="tags">{reference.similarities.map((item, itemIndex) => <li key={itemIndex}>{item}</li>)}</ul>}{reference.reason && <p>{reference.reason}</p>}</li>)}</ul> : <p className="muted">No references added.</p>}</section></div>
...
export function GameLibrary({ api, active, onNavigationGuardChange, onConnectionRepair }: { api: DesktopBridge; active: boolean; onNavigationGuardChange?: (guard: NavigationGuard | null) => void; onConnectionRepair?: () => void }) {
  const [library, setLibrary] = useState<LibraryState>({ search: '', query: '', onlyCollection: false, page: null, busy: false, error: null, failedInput: null });
  ...
  const loadPage = useCallback(async (input: GameListInput, preserve = true) => {
    ...
    setLibrary(previous => ({ ...previous, query: input.query || '', onlyCollection: input.onlyCollection || false, page: preserve ? previous.page : null, busy: true, error: null, failedInput: null }));
    try {
      const response = await api.games.list({ ...input, limit: 24, offset: input.offset || 0 });
      if (!alive.current || generation.current !== token) return;
      if (!response.ok) { setLibrary(previous => ({ ...previous, busy: false, error: response.error, failedInput: input })); return; }
    ...
    } catch { if (alive.current && generation.current === token) setLibrary(previous => ({ ...previous, busy: false, error: interrupted, failedInput: input })); }
  }, [api]);
  ...
  function backToList(refresh = false) {
    ...
    setRoute({ kind: 'list' });
    if (refresh) { const current = listState.current; void loadPage({ query: current.query, onlyCollection: current.onlyCollection, offset: current.page?.offset || 0 }); }
  ...
  function saved(game: GameDetail) {
    ...
    void loadPage({ query: current.query, onlyCollection: current.onlyCollection, offset: current.page?.offset || 0 });
  }
  const page = library.page;
  const shownStart = page && page.items.length ? page.offset + 1 : 0;
  const pageInput = (offset: number): GameListInput => ({ query: library.query, onlyCollection: library.onlyCollection, offset });
  return <div className="games-library">
    <div ref={listRoot} hidden={route.kind !== 'list'}><div className="game-library-tools"><form className="search-form" role="search" onSubmit={event => { event.preventDefault(); void loadPage({ query: library.search.trim(), onlyCollection: library.onlyCollection, offset: 0 }, false); }}><Icon name="search"/><input type="search" aria-label="Search games" maxLength={255} placeholder="Search games…" value={library.search} onChange={event => setLibrary(previous => ({ ...previous, search: event.target.value }))}/><button className="button secondary" type="submit">Search</button></form><button className="button primary" onClick={() => { rememberScroll(); scrollTop(); setRoute({ kind: 'editor', game: null }); }}>New game</button></div>
      <div className="game-list-filter"><label className="collection-filter"><input type="checkbox" checked={library.onlyCollection} onChange={event => void loadPage({ query: library.query, onlyCollection: event.target.checked, offset: 0 }, false)}/><Icon name="collection"/>Saved only</label><button className="text-button" disabled={library.busy} onClick={() => void loadPage(pageInput(page?.offset || 0))}>Refresh</button></div>
      {library.query && <div className="active-filters"><span>Results for “{library.query}”</span><button className="text-button" onClick={() => { setLibrary(previous => ({ ...previous, search: '' })); void loadPage({ onlyCollection: library.onlyCollection, offset: 0 }, false); }}>Clear search</button></div>}
      {page && page.items.length > 0 && <ul className="profile-list" aria-label="Games">{page.items.map(game => <li key={game.id}><button className="profile-row" aria-label={`Open ${titleOf(game)}`} onClick={() => void openGame(game.id)}><Artwork url={game.cover_url} name={titleOf(game)} kind="games"/><span className="profile-row-content"><span className="profile-row-title">{titleOf(game)}{game.favorite && <Icon name="collection"/>}</span><span className="profile-row-summary">{game.developer || game.website_url || 'Manual game'}</span>{game.tags.length > 0 && <span className="row-tags">{game.tags.slice(0, 3).map((tag, index) => <span key={index}>{tag}</span>)}</span>}</span><span className="row-date">{analyzedDate(game.last_analyzed_at)}</span><Icon name="chevron"/></button></li>)}</ul>}
      ...
      {!library.busy && !library.error && page?.items.length === 0 && <EmptyState title={library.query || library.onlyCollection ? 'No matching games' : 'No games yet'} action={library.query || library.onlyCollection ? <button className="button secondary" onClick={() => { setLibrary(previous => ({ ...previous, search: '' })); void loadPage({ query: '', onlyCollection: false, offset: 0 }, false); }}>Clear filters</button> : undefined}/>}
      {page && <div className="game-pagination"><span>{shownStart}–{page.offset + page.items.length} of {page.total}</span><div><button className="button secondary" disabled={library.busy || page.offset === 0} onClick={() => void loadPage(pageInput(Math.max(0, page.offset - 24)))}>Previous page</button><button className="button secondary" disabled={library.busy || page.offset + page.limit >= page.total} onClick={() => void loadPage(pageInput(page.offset + page.limit))}>Next page</button></div></div>}
    </div>
