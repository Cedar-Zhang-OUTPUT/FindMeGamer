...
export function useActivityDrafts({api,activityId,gameId,active,pollMs=4000}:Props){
  ...
  async function register(){if(busy||locked||!game||!catalog||game.steamAppId&&game.steamAppId!=='4952700')return false;return execute({kind:'registerCanonical',gameId:game.id,canonicalFixedHash:catalog.builtin.fixed_hash});}
  ...
  async function create(){
    ...
    if(busy||locked||!active||!original||!template||dirty)return false;
