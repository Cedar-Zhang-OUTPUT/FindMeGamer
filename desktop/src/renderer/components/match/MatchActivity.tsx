...
import {canContinueQuery,taskDate,taskLabel} from './matchStatus';
import type {MatchCommand} from './matchMutation';
...
export function MatchActivity({api,activityId,active,onBack,onOpenCreator,onNavigationGuardChange,onConnectionRepair}:{api:DesktopBridge;activityId:string;active:boolean;onBack:()=>void;onOpenCreator:(id:string,section?:'overview'|'contacts'|'works')=>void;onNavigationGuardChange?:(guard:NavigationGuard|null)=>void;onConnectionRepair?:()=>void}){
  ...
  const guard=useMatchGuard({operation,dirty:draft!==null,onChange:onNavigationGuardChange,onDiscard:()=>setDraft(null)});
  ...
  function startDiscovery(){const conditions=draft??defaultConditions();if(Object.keys(validateConditions(conditions)).length)return;execute({kind:'createPlan',activityId,data:{...conditions,mode:'discover'}});}
  ...
  return <section className="match-activity">
    <div className="match-breadcrumb"><button className="text-button" onClick={()=>guard.request(onBack)}><Icon name="arrow"/>Activities</button><span>{gameName}</span><button className="icon-button" title="Refresh activity" aria-label="Refresh activity" onClick={session.refresh} disabled={session.loading}><Icon name="refresh"/></button></div>
    ...
    {activity&&<>
      ...
      {showingConditions?<><h2>Find creators</h2>{plan&&<p className="inline-warning">New search · Earlier results and evaluations stay in history.</p>}<DiscoveryConditions value={draft??defaultConditions()} onChange={setDraft} disabled={locked} onSubmit={startDiscovery} onCancel={session.scope?.planId||session.scope?.queryId?()=>setDraft(null):undefined}/></>:<>
        ...
        {query&&<>
          <div className="match-discovery-header"><div><strong className="match-found-count">{query.result_count}</strong><span>creators found</span><span className={`status-badge ${running?'match-running':''}`}>{query.stop_requested&&running?'Stopping':taskLabel(query.status)}</span></div>
            {running?<button className="button secondary" disabled={locked||query.stop_requested} onClick={()=>execute({kind:'stop',queryId:query.id})}>Stop discovery</button>:<button className="button secondary" disabled={locked||!canContinueQuery(query)||(query.requires_acknowledgement&&!acknowledge)} onClick={()=>execute({kind:'continueDiscovery',queryId:query.id,data:{acknowledge_unknown:acknowledge}})}>Continue discovery</button>}
          </div>
          ...
          <div className="match-source-states">{Object.entries(query.sources).map(([platform,value])=>{const source=value&&typeof value==='object'&&!Array.isArray(value)?value:null;return <span className="match-source-status" key={platform}>{platform==='youtube'?'YouTube':platform==='x'?'X':taskLabel(platform)} · {taskLabel(typeof source?.status==='string'?source.status:'unknown')}</span>;})}</div>
