...
export function MatchActivity({api,activityId,active,onBack,onOpenCreator,onNavigationGuardChange,onConnectionRepair,onCollectionSettings}:{api:DesktopBridge;activityId:string;active:boolean;onBack:()=>void;onOpenCreator:(id:string,section?:'overview'|'contacts'|'works')=>void;onNavigationGuardChange?:(guard:NavigationGuard|null)=>void;onConnectionRepair?:()=>void;onCollectionSettings?:()=>void}){
  ...
  const credentialsChanged=useCallback(()=>{operation.credentialsChanged();savedOperation.credentialsChanged();setReadVersion.current++;setRecoveredSet(null);setSetReadBusy(false);},[operation.credentialsChanged,savedOperation.credentialsChanged]);
  const guard=useMatchGuard({operation:{busy:operation.busy||savedOperation.busy,locked:operation.locked||savedOperation.locked,state:{error:operation.state.error??savedOperation.state.error},credentialsChanged},dirty:draft!==null||listDraft!==null,readRepair:policyNeedsRepair,onChange:onNavigationGuardChange,onDiscard:()=>{setDraft(null);setListDraft(null);}});
  ...
  const locked=operation.busy||operation.locked||savedOperation.busy||savedOperation.locked;
  ...
  return <section className="match-activity">
    ...
    {activity&&<>
      ...
      <SavedSetPicker api={api.savedSets} activityId={activityId} active={active} refreshToken={setsRefresh} disabled={locked} selectedId={openedSetId} onOpen={openSaved}/>
      ...
      {openedSetId&&<SavedSetResults api={api.savedSets} id={openedSetId} activityId={activityId} active={active} onMetadata={savedMetadata} onOriginal={()=>setOpenedSetId(null)} onOpenCreator={openCreator}/>}
