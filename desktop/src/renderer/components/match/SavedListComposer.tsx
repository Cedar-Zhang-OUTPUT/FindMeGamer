...
export function SavedListComposer({draft,onChange,onCancel,onSave,onMarkLoaded,onClear,disabled,current,loadedIds}:{loadedIds?:string[];draft:SavedListDraft;onChange:(value:SavedListDraft)=>void;onCancel:()=>void;onSave:()=>void;onMarkLoaded:()=>void;onClear:()=>void;disabled:boolean;current:boolean}){
  ...
  return <form className="saved-list-composer" aria-label="Save candidate list" onSubmit={event=>{event.preventDefault();if(valid&&!disabled&&current)onSave();}}>
    ...
    {loadedIds&&<ScopedSelectionActions scope="Loaded saved-list candidates" ids={loadedIds} selected={draft.ids} disabled={disabled||!current} limit={600} onChange={ids=>onChange({...draft,ids})}/>}
    {!loadedIds&&<div className="saved-list-members"><span>{count} marked</span><button type="button" className="text-button" disabled={disabled||!current||count>=600} onClick={onMarkLoaded}>Mark loaded</button><button type="button" className="text-button" disabled={disabled||!count} onClick={onClear}>Clear marks</button></div>}
