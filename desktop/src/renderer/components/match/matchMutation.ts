...
import type {CreatorSearchAccepted} from '../../../shared/creatorSearch';

export type MatchCommand=
  | {kind:'createCreatorSearch';activityId:string;data:PlanCreate}
  | {kind:'stopCreatorSearch'|'appendCreatorSearch';id:string}
  | {kind:'retryCreatorSearch';id:string;data:{acknowledge_unknown?:boolean}}
...
export type MatchReceipt=ActivityView|PlanAccepted|DiscoveryAccepted|DiscoveryStopped|EvaluationAccepted|CreatorSearchAccepted;
...
export function matchRejected(code:string){return rejected.has(code)||['search_running','search_not_retryable','search_outcome_unknown','search_not_found'].includes(code);}
...
export function dispatchMatch(api:MatchAPI,attempt:MatchAttempt):Promise<Result<MatchReceipt>> {
  ...
  switch(command.kind){
    case 'createCreatorSearch':return api.createCreatorSearch({activityId:command.activityId,data:command.data,idempotencyKey});
    case 'stopCreatorSearch':return api.stopCreatorSearch({id:command.id,idempotencyKey});
    case 'retryCreatorSearch':return api.retryCreatorSearch({id:command.id,data:command.data,idempotencyKey});
    case 'appendCreatorSearch':return api.appendCreatorSearch({id:command.id,idempotencyKey});
