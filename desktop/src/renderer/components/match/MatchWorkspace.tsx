...
function MatchWorkspaceContent({api,active,onNavigationGuardChange,onConnectionRepair,onCollectionSettings,onSMTPSettings,gameRequest,onGameRequestHandled,surface='match',onShowMatch,onShowOutreach}:MatchWorkspaceProps){
  ...
  const contextTools=<div className="workspace-tools" role="group" aria-label="Workspace tools"><button className="button secondary" onClick={analysis.showTasks}>Analysis tasks</button>{surface==='outreach'&&onSMTPSettings&&<button className="button secondary" onClick={onSMTPSettings}>Email settings</button>}</div>;
  return <div className="match-workspace">
    {(route.kind==='new'||(overlay&&overlay.kind!=='detail'))&&<div className="analysis-entry-actions">{contextTools}</div>}
    <div ref={listRoot} hidden={route.kind!=='list'}>
      <div className="page-heading match-workspace-heading"><h1>{surface==='outreach'?'Outreach':'Match'}</h1><div className="workspace-heading-actions">{contextTools}{surface==='outreach'?<button className="button primary" onClick={onShowMatch}>Find creators</button>:<button className="button primary" onClick={()=>{listScroll.current=scroller()?.scrollTop??0;setRoute({kind:'new'});}}>New activity</button>}</div></div>
    ...
    {route.kind==='activity'&&<><div ref={activityRoot} hidden={Boolean(overlay)}><MatchActivity key={route.id} api={api} activityId={route.id} active={active&&!overlay} surface={surface} onShowMatch={onShowMatch} onShowOutreach={onShowOutreach} onBack={toList} onOpenCreator={(id,section)=>void openCreator(id,surface==='outreach'?'invitations':section==='contacts'?'emails':section==='works'?'works':'profile')} onNavigationGuardChange={activityGuard} onConnectionRepair={onConnectionRepair} onCollectionSettings={onCollectionSettings} onSMTPSettings={onSMTPSettings} contextTools={contextTools}/></div>
      {overlay&&<div className="match-creator-route">{overlay.kind!=='detail'&&<button className="text-button back-button" onClick={requestBack}><Icon name="arrow"/>Back to activity</button>}
        ...
        {overlay.kind==='detail'&&<>{overlay.saved&&<p role="status" className="creator-saved-message">Saved</p>}<CreatorRecord key={overlay.creator.id} api={api} creator={overlay.creator} initialSection={overlay.section} invitationActivityId={route.id} refreshToken={overlay.creator.revision} onBack={requestBack} backLabel="Back to activity" onEdit={edit} contextTools={contextTools}/></>}
