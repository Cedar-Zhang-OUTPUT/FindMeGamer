import {SelectionActions,ScopedSelectionActions} from '../SelectionActions';
...
export function OutreachNotice({controller:c,onRepair}:{controller:Controller;onRepair?:()=>void}){
  ...
  return <div className="outreach-feedback">
    {c.local.error&&<ErrorNotice error={c.local.error} onRetry={c.local.state?()=>void c.local.retryPersistence():undefined}/>}{c.local.state?.error&&<ErrorNotice error={c.local.state.error}/>}
    {c.local.enabled&&c.local.busy&&<Loading label="Preparing selected creators…"/>}
    {c.local.enabled&&c.local.locked&&!c.local.busy&&<div className="button-row">{c.local.state?.stage==='conflict'?<button className="button secondary" onClick={()=>void c.session.refresh().then(rows=>rows&&c.local.reviewConflict(rows))}>Review changed selections</button>:<><button className="button secondary" onClick={()=>void c.prepare([])}>Retry original preparation</button><button className="text-button" onClick={()=>void c.local.checkSaved()}>Check saved preparation</button></>}</div>}
...
export function OutreachWorkspace({api,controller:c,active,onRequest,onOpenCreator,onChooseTemplate,evaluationChoices=[]}:{api:DesktopBridge;controller:Controller;active:boolean;onRequest:(action:()=>void)=>void;onOpenCreator:(id:string,section?:'overview'|'contacts'|'works')=>void;onChooseTemplate?:()=>void;evaluationChoices?:Array<{id:string;label:string}>}){
  ...
  // Retain this review's displayed scope after deselection so it can be selected again.
  const reviewIds=useRef<string[]>([]);
  if(!selected||!c.local.state)reviewIds.current=[];
  else reviewIds.current=[...new Set([...reviewIds.current,...c.local.desired])].filter(id=>c.local.state!.draft.members[id]);
  const reviewMembers=reviewIds.current.map(id=>c.local.state!.draft.members[id]);
  const selectableReview=reviewMembers.filter(member=>!c.session.items.some(row=>(row.id===member.observed?.selectionId||row.candidate_id===member.candidateId)&&row.identity_changed));
  if(selected&&c.local.enabled)return <section className="outreach-workspace" aria-label="Selected people"><header className="outreach-heading"><button className="text-button" onClick={()=>onRequest(()=>{c.setPanel('candidates');c.setSelectedId(null);})}>Back to candidates</button><h2>Selected · {c.local.desired.length}</h2><div className="button-row"><button className="button primary" disabled={c.busy||c.locked||!c.local.ready||!c.local.desired.length||c.local.desired.length>600} onClick={()=>void c.prepare([])}>Prepare {c.local.desired.length}</button><span className="muted">On this Mac · not sent</span></div></header>
    {c.local.loading&&<Loading label="Loading local choices…"/>}
    {!c.local.desired.length&&!c.local.loading&&<button className="button secondary" onClick={()=>c.setPanel('candidates')}>Choose creators</button>}
    <SelectionActions scope="Listed review creators" selectDisabled={!selectableReview.some(member=>!c.local.desired.includes(member.candidateId))} count={reviewMembers.length} selected={reviewMembers.filter(member=>c.local.desired.includes(member.candidateId)).length} disabled={c.busy||c.locked||!c.local.ready} onSelect={()=>c.local.changeMany(selectableReview,true)} onClear={()=>c.local.changeMany(reviewMembers,false)}/>
    <ul className="local-selection-review" aria-label="Selected creators">{reviewIds.current.map(id=>{const member=c.local.state!.draft.members[id];return <li key={id}><input type="checkbox" aria-label={`Select ${member.name||member.identity.account_id} in review`} checked={c.local.desired.includes(id)} disabled={c.busy||c.locked||!c.local.ready||(!c.local.desired.includes(id)&&!selectableReview.includes(member))} onChange={event=>c.local.change(member,event.target.checked)}/><div><strong>{member.name||member.identity.account_id}</strong><span className="review-table-muted">{member.identity.platform} · {member.identity.account_id}</span></div><div className="button-row"><button className="text-button" disabled={c.busy||c.locked} onClick={()=>onOpenCreator(member.creatorId)}>View creator</button><button className="text-button" disabled={c.busy||c.locked} aria-label={`Remove ${member.name||member.identity.account_id} from outreach`} onClick={()=>c.local.change(member,false)}>Remove</button></div></li>;})}</ul>
  </section>;
  return <section className="outreach-workspace" aria-label={selected?'Selected people':'Outreach preparation'}>
    ...
    <div className={`outreach-columns ${person?'has-detail':''}`}>
      <div className="outreach-roster">{selected&&people.length>0&&<ScopedSelectionActions scope="Loaded preparation people" ids={people.map(p=>p.id)} selected={c.chosenIds} disabled={disabled} limit={600} onChange={c.setChosenIds}/>}
