...
export function useMatchGuard({operation,dirty,enabled=true,onChange,onDiscard}:{operation:ReturnType<typeof useMatchOperation>;dirty:boolean;enabled?:boolean;onChange?: (guard:NavigationGuard|null)=>void;onDiscard:()=>void}){
  ...
  const guard=useMemo<NavigationGuard>(()=>{
    ...
    if(operation.state.error||operation.locked)handler.recovery={credentialsChanged:operation.credentialsChanged};
    return handler;
  },[operation.state.error,operation.locked,operation.credentialsChanged]);
