...
export type SearchReadErrors=Partial<Record<'people'|'results'|'candidates',PublicError>>;
const inconsistentPage:PublicError={code:'search_results_changed',message:'Search results changed during loading. Reload results to read a consistent version.',retryable:true};
const readFailure=(cause:unknown):PublicError=>{
  if(cause&&typeof cause==='object'&&'code' in cause&&'message' in cause&&'retryable' in cause)return cause as PublicError;
  return {code:'search_read_failed',message:'Could not load this part of the search. Reload to try again.',retryable:true};
};
async function all<T extends {candidate_id:string}>(fetch:(offset:number)=>Promise<Result<MatchPage<T>>>):Promise<T[]>{
  ...
  for(let offset=0;offset<600;offset+=100){const r=await fetch(offset);if(!r.ok)throw r.error;if(r.data.offset!==offset||r.data.total>600||(total!==undefined&&r.data.total!==total))throw inconsistentPage;total=r.data.total;if(r.data.items.length!==Math.min(100,Math.max(0,total-offset)))throw inconsistentPage;items.push(...r.data.items);if(new Set(items.map(item=>item.candidate_id)).size!==items.length)throw inconsistentPage;if(items.length===total)return items;}
  throw inconsistentPage;
}
export function useCreatorSearchSession(api:MatchAPI,activityId:string,active:boolean,pollMs=2500){
  ...
  const [readErrors,setReadErrors]=useState<SearchReadErrors>({});
  const initialized=useRef(false),version=useRef(0);
  const refresh=useCallback(()=>setEpoch(v=>v+1),[]);
  const select=useCallback((id:string|null)=>{version.current++;setScope(id?{kind:'search',id}:{kind:'legacy'});setSearch(null);setResults([]);setPeople([]);setCandidates([]);setCurrent(false);setError(null);setReadErrors({});},[]);
  ...
  useEffect(()=>{
    ...
    async function poll(){setLoading(true);setCurrent(false);let keepPolling=true;
      try{const response=await api.creatorSearch(id);if(!valid())return;if(!response.ok)throw response.error;const task=response.data;if(task.activity_id!==activityId)throw matchReadError;setSearch(task);setHistory(h=>h.some(s=>s.id===id)?h.map(s=>s.id===id?task:s):[task,...h]);keepPolling=creatorSearchRunning(task);
        setError(null);
        if(!keepPolling){
          // Each source commits independently. Selection requires the complete snapshot.
          async function read<T>(key:keyof SearchReadErrors,request:()=>Promise<T>,apply:(value:T)=>void){
            try{const value=await request();if(!valid())return false;apply(value);setReadErrors(previous=>{const next={...previous};delete next[key];return next;});return true;}
            catch(cause){if(valid())setReadErrors(previous=>({...previous,[key]:readFailure(cause)}));return false;}
          }
          const ready=await Promise.all([
            read('people',()=>all(offset=>api.creatorSearchPeople({id,offset,limit:100})),setPeople),
            read('results',()=>task.evaluation_id?all(offset=>api.evaluationResults({id:task.evaluation_id!,offset,limit:100})):Promise.resolve([]),setResults),
            read('candidates',()=>task.query_id?readCandidateMembership(api,task.query_id,{evidence:'all',sort:'added'}):Promise.resolve([]),setCandidates),
          ]);
          if(!valid())return;
          setCurrent(ready.every(Boolean));
        }
        else setCurrent(false);
      }catch(cause){if(valid()){setCurrent(false);setError(readFailure(cause));}keepPolling=false;}
  ...
  return{scope,search,history,historyTotal,historyError,error,readErrors,loading,current,people,results,candidates,select,refresh,loadOlder:()=>setHistoryCapacity(v=>v+50)};
}
