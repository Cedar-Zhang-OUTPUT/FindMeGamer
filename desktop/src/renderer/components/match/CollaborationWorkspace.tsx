...
export function CollaborationWorkspace({controller:c,active,onOpenCreator,onChoosePeople,onConnectionRepair}:{controller:ReturnType<typeof useActivityCollaboration>;active:boolean;onOpenCreator:(id:string)=>void;onChoosePeople:()=>void;onConnectionRepair?:()=>void}){
 ...
 return <section hidden={!active} className="collaboration-workspace" aria-label="Invitations">
  ...
  <div className="collaboration-layout"><aside aria-label="Invitation list"><ul aria-label="Invitation relationships">{c.page?.items.map(row=><li key={row.selection_id}><button aria-pressed={c.selectedId===row.selection_id} aria-controls={detailId} disabled={c.busy||c.locked} onClick={()=>c.select(row.selection_id)}><span className="collaboration-roster-person"><strong>{row.display_name||'Unnamed creator'}</strong><small>{row.identity.platform==='youtube'?'YouTube':row.identity.platform==='x'?'X':typeof row.identity.platform==='string'?row.identity.platform:'Platform unknown'} · {recordedWork(row)}{!row.selected?' · Historical':''}</small></span><span aria-label={`Sending: ${label(row.sending_state)}`} className={`collaboration-state sending-${row.sending_state}`}>{label(row.sending_state)}</span><span aria-label={`Response: ${label(row.invitation_state)}`} className={`collaboration-state response-${row.invitation_state}`}>{label(row.invitation_state)}</span><span aria-label={`Follow-up: ${label(row.follow_up_state)}`} className="collaboration-state">{label(row.follow_up_state)}</span></button></li>)}</ul>
  ...
  </aside><section id={detailId} className="collaboration-detail" aria-label="Current invitation">{recovery}{c.invitation&&c.invitation.selection_id!==c.selectedId&&!c.error&&<Loading label="Loading relationship…"/>}{c.invitation?<CollaborationEditor key={c.editorEpoch} invitation={c.invitation} current={active&&c.current} busy={c.busy||c.locked} confirmedChange={c.confirmedChange} onUpdate={c.update} onRespond={c.respond} onDirtyChange={c.setDirty} onOpenCreator={()=>onOpenCreator(c.invitation!.creator_id)}/>:c.selectedId?<Loading label="Loading relationship…"/>:null}</section></div>
 </section>;
}
