...
export function useMatchOperation(api:MatchAPI){
  ...
  useEffect(()=>{
    ...
    const remaining=state.attempt.startedAt+MATCH_RETRY_WINDOW-Date.now();
  ...
  const dispatch=useCallback(async(attempt:MatchAttempt,replay:boolean):Promise<OperationReceipt|null>=>{
    ...
    try{
      ...
      if(!replay&&matchRejected(response.error.code))update({phase:'idle',error:response.error});
  ...
  const credentialsChanged=useCallback(()=>{const latest=current.current;if(latest.phase!=='idle')update({...latest,attempt:{...latest.attempt,connectionChanged:true}});},[update]);
  const confirmSaved=useCallback(()=>{if(!inFlight.current)update({phase:'idle',error:null});},[update]);
  return {state,execute,retry,credentialsChanged,confirmSaved,busy:state.phase==='running',locked:state.phase==='uncertain',retryAllowed:state.phase==='uncertain'&&canRetryMatch(state.attempt)};
}
