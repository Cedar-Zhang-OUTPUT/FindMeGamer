...
export function LibraryView({ api, active }: { api: DesktopBridge; active: boolean }) {
  const library = useLibrary(api, true, active);
  const { state, kind } = library;
  ...
  function switchTab(target: ProfileKind) { library.setKind(target); }
  return <>
    <div ref={list} hidden={selection !== null}>
      <div className="page-heading"><div><span className="eyebrow">Your workspace</span><h1>Library</h1></div><span className="read-only-label">Read-only</span></div>
      <div className="library-navigation"><div className="segmented" role="tablist" aria-label="Library type">{(['creators', 'games'] as const).map(target => <button key={target} role="tab" id={`tab-${target}`} aria-controls={`panel-${target}`} aria-selected={kind === target} tabIndex={kind === target ? 0 : -1} onClick={() => switchTab(target)} onKeyDown={event => {
        if (['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) { event.preventDefault(); const next = event.key === 'Home' ? 'creators' : event.key === 'End' ? 'games' : kind === 'creators' ? 'games' : 'creators'; switchTab(next); document.getElementById(`tab-${next}`)?.focus(); }
      }}>{target === 'creators' ? 'Creators' : 'Games'}</button>)}</div><div className="platforms">{kind === 'creators' ? <><span className="platform-label"><span className="platform-dot youtube"/>YouTube</span><span className="platform-unavailable">Instagram <span>Not connected</span></span></> : <span className="platform-label">Steam</span>}</div></div>
      <div role="tabpanel" id={`panel-${kind}`} aria-labelledby={`tab-${kind}`}>
        <div className="library-toolbar"><form className="search-form" role="search" onSubmit={event => { event.preventDefault(); library.search(); }}><Icon name="search"/><input type="search" aria-label={`Search ${kind}`} placeholder={`Search ${kind}…`} value={state.draft} onChange={event => library.setDraft(event.target.value)}/><button className="button primary" type="submit">Search</button></form><label className="collection-filter"><input type="checkbox" checked={state.onlyCollection} onChange={event => library.setCollection(event.target.checked)}/><Icon name="collection"/>Saved only</label><button className="icon-button refresh-button" title="Refresh Library" aria-label="Refresh Library" onClick={library.refresh} disabled={state.phase !== 'idle'}><Icon name="refresh"/></button></div>
        ...
        {state.phase === 'loading' ? <div className="list-loading"><Loading label={`Loading ${kind}…`}/><div className="skeleton-rows" aria-hidden="true"><span/><span/><span/></div></div> : <>
          {state.items.length > 0 && <><div className="list-caption"><span>{state.items.length} {kind}{state.nextCursor ? ' loaded' : ''}</span><span>Analyzed</span></div><ul className="profile-list" aria-label={kind === 'creators' ? 'Creators' : 'Games'}>{state.items.map(profile => <li key={profile.id}>
          ...
          {!state.error && state.loaded && state.items.length === 0 && <EmptyState title={state.query || state.onlyCollection ? 'No matching profiles' : `No ${kind} yet`} action={(state.query || state.onlyCollection) ? <button className="button secondary" onClick={library.clearFilters}>Clear filters</button> : undefined}>{state.query || state.onlyCollection ? undefined : 'Profiles saved in your workspace appear here.'}</EmptyState>}
